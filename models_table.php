...
if (isset($_SESSION['username']) && isset($_SESSION['password'])) {
    ...
    $data = $connection->prepare("SELECT models.*, categories.name AS cat_name FROM models 
    ...
    <div style="display: flex">
        ...
        <div class="container mt-5">
            ...
            <table class="mb-4 table table-bordered table-dark" id="table_data">
                ...
                $data = $connection->prepare("SELECT models.*, categories.name AS cat_name FROM models
                    LEFT JOIN categories ON models.categories_id = categories.id ORDER BY id DESC LIMIT $this_page_first_result,$result_per_page ");
                $data->execute();
                $result = $data->fetchAll(PDO::FETCH_ASSOC);
                ...
                <?php foreach ($result as $x): ?>
                    <tr>
                        ...
                        <td class="update"><a href="updatefieldmodel.php?id=<?= $x['id'] ?>&name=<?= $x['name'] ?>&category_id=<?= $x['categories_id'] ?>">↻</a>
                        </td>
                    </tr>
    ...
    <script>
        $('.deleteF').on('click', function () {
            ...
            if (conf) {
                $(this).attr('href', 'beckend/delete_model.php?id=' + del_id);
            }
        })
    </script>
